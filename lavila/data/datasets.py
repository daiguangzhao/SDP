...
import pandas as pd
import torch
from lavila.models.text_prompt import text_prompt
...
class VideoCaptionDatasetBase(torch.utils.data.Dataset):
    def __init__(self, dataset, root, metadata,  is_trimmed=True, use_few_shot = False, shot=1, use_text4vis=False):
        ...
        if self.dataset == 'ego4d':
            ...
            with open(metadata) as f:
                ...
                _ = next(csv_reader)  # skip the header 跳过CSV文件的第一行（表头）
                for row in csv_reader:
                    ...
                    narration = row[8]
                    verb, noun = int(row[10]), int(row[12]) # 是数字
                    if use_few_shot:
                        vn = '{}:{}'.format(int(row[10]), int(row[12]))
                    verb_caption, noun_caption = str(row[9]), str(row[11])
                    vid_path = '{}/{}.MP4'.format(pid, vid) # e.g., P01/P01_01.MP4
                    fps = fps_dict[osp.join(self.root, vid_path)]
                    start_frame = int(np.round(fps * start_timestamp)) # 计算起第起始帧数和第终止帧
                    end_frame = int(np.ceil(fps * end_timestamp))
                    if use_few_shot:
                        self.samples_fs.append((vid_path, start_frame, end_frame, narration, verb, noun, verb_caption, noun_caption, vn))
                    else:
                        self.samples.append((vid_path, start_frame, end_frame, narration, verb, noun, verb_caption, noun_caption))
            if use_few_shot:
                for video_clip in self.samples_fs:
                    if video_clip[-1] not in cls_dict:
                        cls_dict[video_clip[-1]] = [video_clip]
                    else:
                        cls_dict[video_clip[-1]].append(video_clip)
                select_vids = []
                for category, v in cls_dict.items():
                    slice = random.sample(v, shot)
                    select_vids.extend(slice)
                # n_repeat = len(self.samples_fs) // len(select_vids) # 可重复可不重复
                self.samples_fs = select_vids
            if self.use_text4vis:
                verb_caption_all, noun_caption_all, action_caption_all = [], [], []
                for video_clip in self.samples_fs:
                    verb_caption_all.append(video_clip[-3])
                    noun_caption_all.append(video_clip[-2])
                    action_caption_all.append(video_clip[3])
                v_classes, _, _ = text_prompt(verb_caption_all)
                n_classes, _, _ = text_prompt(noun_caption_all)
                a_classes, _, _ = text_prompt(action_caption_all)                
                self.verb_caption = v_classes
                self.noun_caption = n_classes
                self.action_caption = a_classes               
    ...
    def get_raw_item(self, i, is_training=True, num_clips=1, clip_length=32, clip_stride=2, sparse_sample=False,
        ...
        if self.dataset == 'ego4d':
            ...
            if self.use_few_shot:
                vid_path, start_frame, end_frame, narration, verb, noun, verb_caption, noun_caption, _ = self.samples_fs[i]               
                frame_ids = get_frame_ids(start_frame, end_frame, num_segments=clip_length, jitter=is_training)
                frames = video_loader_by_frames(self.root, vid_path, frame_ids)
                return frames, '{}:{}'.format(verb, noun), (verb_caption, noun_caption)       
            else:
                vid_path, start_frame, end_frame, narration, verb, noun, verb_caption, noun_caption = self.samples[i]               
                frame_ids = get_frame_ids(start_frame, end_frame, num_segments=clip_length, jitter=is_training)
                frames = video_loader_by_frames(self.root, vid_path, frame_ids)
                return frames, '{}:{}'.format(verb, noun), (verb_caption, noun_caption)
        elif self.dataset == 'egtea':
...
class VideoClassyDataset(VideoCaptionDatasetBase):
    def __init__(
        ...
        is_trimmed=True, 
        caption = False, # 自定义
        hard_prompt_as_actionclip=False,
        hard_prompt_as_xclip=False,
        csc_text_prompt = False,
        use_few_shot = False,
        shot = 1,
        use_text4vis = False
       
    ):
        super().__init__(dataset, root, metadata, is_trimmed=is_trimmed, use_few_shot=use_few_shot, shot=shot, use_text4vis = use_text4vis)
        ...
        self.tokenizer = tokenizer
        self.hard_prompt_as_actionclip = hard_prompt_as_actionclip
        self.hard_prompt_as_xclip = hard_prompt_as_xclip
    ...
    def __getitem__(self, i):
        if self.caption:
            if self.csc_text_prompt:
                frames, label, caption = self.get_raw_item(
                    i, is_training=self.is_training,
                ...
                v_caption, n_caption = caption
                # verb_gt_class_name, noun_gt_class_name = class_name
            else:            
                frames, label, caption = self.get_raw_item(
                    i, is_training=self.is_training,
                    num_clips=self.num_clips,
                ...
                if isinstance(caption, tuple):
                    v_caption, n_caption = caption
                    if self.hard_prompt_as_actionclip:
                        v_caption = f"The dynamic motion of human action is {v_caption}"
                        n_caption = f"The interacting object of human action is {n_caption}"
                    elif self.hard_prompt_as_xclip:
                        text_aug = f"{{}}" # 实际上，等于没有替换
                        v_caption = text_aug.format(v_caption)
                        n_caption = text_aug.format(n_caption)
                    if self.tokenizer is not None:
                        # noun_caption = self.tokenizer(noun_caption)
                        verb_caption = self.tokenizer(v_caption)
                        noun_caption = self.tokenizer(n_caption)
        else:
        ...
        if self.label_mapping is not None:
            if isinstance(label, list):
                ...
                label = self.label_mapping[label] #在epic-cls中是字典

        # print("===> orginal_noun_caption->label {}->{}:".format(noun_caption_, label)) # label返回的是自定义的action的值 int类型 小于3806
        # print("frames.size()", frames.size()) ----> torch.Size([3, 16, 224, 224])
        if self.caption:
            if self.is_training:
                if self.csc_text_prompt:
                    return frames, label, v_caption, n_caption
                elif self.use_few_shot:
                   return frames, label, verb_caption, noun_caption                    
                else:    
                    return frames, label, verb_caption, noun_caption
                # print("===> orginal_noun_caption->label {}->{}:".format(noun_caption_, label)) # label返回的是自定义的action的值 int类型 小于3806
            else:
...
def get_downstream_dataset(transform, tokenizer, args, subset='train', label_mapping=None):
    if subset == 'train':
        return VideoClassyDataset(
            args.dataset, args.root, args.metadata_train, 
            transform,
            is_training=True, 
            tokenizer=tokenizer,
            ...
            hard_prompt_as_actionclip=args.hard_prompt_as_actionclip,
            hard_prompt_as_xclip=args.hard_prompt_as_xclip,
