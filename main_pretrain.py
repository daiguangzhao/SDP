...
def main(args):
    ...
    model = getattr(models, args.model)(
        ...
        freeze_lm_vclm=args.freeze_lm_vclm, # True
        freeze_visual_vclm=args.freeze_visual_vclm, # True
        freeze_visual_vclm_temporal=args.freeze_visual_vclm_temporal, # True
        num_frames=args.clip_length,
        drop_path_rate=args.drop_path_rate,
        temperature_init=args.temperature_init, # 0.07
    )
    ...
    for n, p in model.named_parameters():
        if not p.requires_grad:
            continue  # frozen weights # 跳过不需要梯度计算的参数
