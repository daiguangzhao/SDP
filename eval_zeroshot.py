...
from lavila.models.openai_clip import tokenize as tokenizer
import csv 
...
def get_args_parser():
    ...
    parser.add_argument('--distributed', default=True, type=bool, help='GPU id to use.')
...
def main(args):
    ...
    # print("=> loaded resume checkpoint '{}' (epoch {}, best_metric = {})".format(args.resume, ckpt['epoch'], ckpt['best_acc1'])) #对于baseline而言，是没有epoch这个字典

    torch.backends.cudnn.benchmark = True

    if args.dataset in ['ek100_cls', 'charades_ego', 'egtea']:
        labels, mapping_vn2act, verb_labels, noun_labels = generate_label_map(args.dataset)
        # noun_labels = Parse_MetaVN(args.metadata_noun)
        # verb_labels = Parse_MetaVN(args.metadata_verb)
    else:
        mapping_vn2act = None
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=int(args.batch_size*8), shuffle=False,
    ...
    if args.dataset in ['ek100_cls', 'charades_ego', 'egtea']:
        ...
        preds_1, targets_1 = validate_zeroshot(val_loader, templates, verb_labels, model, tokenizer)
        preds_2, targets_2 = validate_zeroshot(val_loader, templates, noun_labels, model, tokenizer)
    if args.dataset == 'ek100_cls':
        if args.use_half:
            preds = preds.float()
            preds_2 = preds_1.float()
            preds_2 = preds_2.float()
        top1_a, top5_a = accuracy(preds, targets, topk=(1, 5))
        top1_v, top5_v = accuracy(preds_1, targets_1, topk=(1, 5))
        top1_n, top5_n = accuracy(preds_2, targets_2, topk=(1, 5))

        print('top1_a = {:.3f}'.format(top1_a.item()))
        print('top5_a = {:.3f}'.format(top5_a.item()))
        print('top1_v = {:.3f}'.format(top1_v.item()))
        print('top5_v = {:.3f}'.format(top5_v.item()))
        print('top1_n = {:.3f}'.format(top1_n.item()))
        print('top5_n = {:.3f}'.format(top5_n.item())) 
    ...
    with torch.no_grad():
        ...
        for label in labels:
            if isinstance(label, list):
                texts = [tmpl.format(lbl) for tmpl in templates for lbl in label] # 遍历出去还是原始的文本标签列表
            else:
            ...
            if isinstance(texts, tuple):
                ...
                texts = texts.cuda(non_blocking=True) # 文本特征
                masks = None
